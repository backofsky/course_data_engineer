...
## Практическая работа №13. Практика Airflow
...
![task_1](pictures/task1.png)
![task_2](pictures/task2.png)
...
### Необходимые шаги для работы проекта

1. Развернуть Airflow, используя файл docker-compose.yaml
2. В Airflow создать 2 соединения (connections)
    * conn1  
        Тип соединения: postgres  
        Host: 172.17.0.1 (для linux)  
        Schema: airflow  
        Login:  airflow  
        Password: airflow  
        Port: 5431  
...
3. Cоздать переменную (variables)   
    key: file4numeros  
    val: /opt/airflow/numeros.txt  

    Или импортировать ее из variables.json  
